...
from soaplib.serializers.primitive import String, Integer, Boolean, Decimal, Double
...
class FindProbesInput(ClassSerializer):
    ...
    maskingflag = Integer(min_occurs=1,nillable=False)
    species = String(min_occurs=1,nillable=False)
    targetTm = Double(min_occurs=1,nillable=False)
...
class FISHProbeDesigner(DefinitionBase):
    ...
    def design_probes(self,input):
        ...
        results = find_probes.designV3_1(inseq,
                                    input.noligos,  
                                    input.oligo_length,
                                    input.spacer_length,
                                    input.maskingflag,
...
if __name__=='__main__':
    try:
        ...
        server = make_server('158.130.146.251', 8081, Application([FISHProbeDesigner], 'tns'))
        server.serve_forever()
        """
        design_app = ValidatingApplication([FISHProbeDesigner],'tns')
        from cherrypy.wsgiserver import CherryPyWSGIServer
        server = CherryPyWSGIServer(('158.130.146.251',8082),
                                    design_app,
