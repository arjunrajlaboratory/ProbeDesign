function [outstructure,oligos] = findprobesHD_mask(infile,varargin)
...
p.addParamValue('externalmask',[],@islogical);
...
for i = 1:goodlen
  ...
  if i <= length(p.Results.externalmask)
      goodness(i) = goodness(i) + 100000*(~p.Results.externalmask(i));
  end;
end;
percentgc = goodness;
